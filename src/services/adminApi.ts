...
export const adminApi = {
  ...
  async getReviews(params?: { skip?: number; limit?: number; product_id?: number }): Promise<Review[]> {
    const searchParams = new URLSearchParams();
    if (params?.skip) searchParams.append('skip', params.skip.toString());
    if (params?.limit) searchParams.append('limit', params.limit.toString());
  ...
  async getProductReviews(product_id: number, params?: { skip?: number; limit?: number }): Promise<Review[]> {
    const searchParams = new URLSearchParams();
    if (params?.skip) searchParams.append('skip', params.skip.toString());
    if (params?.limit) searchParams.append('limit', params.limit.toString());
    
    const response = await fetch(`${API_BASE_URL}/reviews/product/${product_id}?${searchParams.toString()}`, {
      headers: getAuthHeaders(),
    });
